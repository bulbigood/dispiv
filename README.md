...
Вся документация хранится в репозитории с жестким разделением форматов для экономии токенов и контроля фокуса у ИИ.
Примеры документов расположены в [/docs](/docs).

### ADRs
* `/docs/adrs/` (Markdown): Исторический контекст и причины решений.

### Specs
* `/docs/specs/` (Markdown + YAML): Монолитные спецификации текущего состояния с кросс-ссылками. Используются как эталон для планирования (Plan), внедрения (Implement) и проверки (Verify).
...
* Idea: demand.md + /docs/specs/ (для понимания текущих ограничений) + /docs/adrs/ (для учета прошлого опыта и ошибок).
* Spec: idea.md (выбранный вектор) + /docs/specs/ (текущие контракты). Исходный код изолирован.
* Plan: Все измененные целевые .spec.md + только декларации/интерфейсы затронутых файлов кода.
* Implement: Соответствующие задаче .spec.md + один конкретный файл задачи task-X.md (из общего плана) + исходный код файлов, подлежащих изменению.
...
3. **Не полностью описана документация** Требуется описать структуру директорий и формата файлов для ADRs, Specs и Archive. Также описать возможности по интеграции других способов документирования.
